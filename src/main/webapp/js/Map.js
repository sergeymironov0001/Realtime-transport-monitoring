...
(function() {
	...
	function createAddFinishTraceMarkerOnClickControl() {
		var handler = function(control, evt) {
			finishTracePosition = convertViewPortPositionToMapPosition(map,
					evt.xy)
			...
			var url = createRouteConstructUrl(startTracePosition,
	...
	function createAddStartTraceMarkerOnClickControl() {
		var handler = function(control, evt) {
			startTracePosition = convertViewPortPositionToMapPosition(map,
					evt.xy)
	...
	/**
	 * Variables startTracePoint, finishTracePoint and constructRouteType need
	 * to exists
	 */
	function createConstructTwoPointTraceControl(map, markers) {
		var handler = function(control, evt) {
			var point = convertViewPortPositionToMapPosition(map, evt.xy)
			var marker = createMarker(map, 12, point);
			addMarker(marker, markers);
			control.deactivate();
			startTracePoint = point;

			var secondClickHandler = function(control, evt) {
				var point = convertViewPortPositionToMapPosition(map, evt.xy)
				var marker = createMarker(map, 12, point);
				addMarker(marker, markers);
				control.deactivate();
				finishTracePoint = point;
				var url = createRouteConstructUrl(startTracePoint,
						finishTracePoint, constructRouteType, "true");
				outputRoute(map, "Route", url, new OpenLayers.Format.KML());
			}
			var secondClickCotrol = createOneClickControl(secondClickHandler);
			map.addControl(secondClickCotrol);
			secondClickCotrol.activate();
		}
		return createOneClickControl(handler);
	}
	...
	function initControls() {
		...
		// ------------Routing------------------
		// addFinishTraceMarkerOnClickControl =
		// createAddFinishTraceMarkerOnClickControl();
		// map.addControl(addFinishTraceMarkerOnClickControl);

		// addStartTraceMarkerOnClickControl =
		// createAddStartTraceMarkerOnClickControl();
		...
		var constructTwoPointTraceControl = createConstructTwoPointTraceControl(map, markers);
		map.addControl(constructTwoPointTraceControl);
		// ��������� ���������� ��� ������� ������ ��������� ��������
		document.getElementById("constructFootTrace").onclick = function() {
			constructRouteType = "foot";
			constructTwoPointTraceControl.activate();
		}
		document.getElementById("constructBicycleTrace").onclick = function() {
			constructRouteType = "foot";
			constructTwoPointTraceControl.activate();
		}
		document.getElementById("constructCarTrace").onclick = function() {
			constructRouteType = "motorcar";
			constructTwoPointTraceControl.activate();
		}
	}
